// Fetch residents and populate table
function fetchResidents() {
  fetch('./backend/residents/fetch.php')
    .then(res => res.json())
    .then(data => {
      ...
      document.getElementById('peopleLength').textContent = data.length;
    });
}
...
document.getElementById('addInfoForm').onsubmit = function(e) {
  ...
  fetch('./backend/residents/create.php', {
    method: 'POST',
    body: formData
  })
  .then(res => res.json())
  .then(() => {
    fetchResidents();
    document.querySelector('#addInfoModal .btn-close').click();
    this.reset();
  });
};
...
function bindEventListeners() {
  ...
  if (barangaySelect) {
    barangaySelect.addEventListener('change', debounce(handleBarangayFilter, 200));
  }
  ...
  // Auto-fill birthday based on age
  const ageInput = document.querySelector('#addInfoForm input[name="age"]');
  const birthdayInput = document.querySelector('#addInfoForm input[name="birthday"]');
  if (ageInput && birthdayInput) {
    // When age is input, set birthday
    ageInput.addEventListener('input', function() {
      const age = parseInt(ageInput.value, 10);
      if (!isNaN(age) && age > 0 && age < 130) {
        const today = new Date();
        const birthYear = today.getFullYear() - age;
        // Set birthday to today's month and day for more accuracy
    ...
    // When birthday is input, set age
...
function loadInitialData() {
  try {
    // Start with an empty residentsData array by default
    residentsData = [];
...
/**
 * Load residents data from server
 */
function loadResidents() {
  fetch('./backend/get_residents.php')
...
function renderResidentsTable(data) {
  try {
    ...
    const tableRows = data.map((resident, index) => {
      ...
      return `
        <tr data-index="${index}" class="table-row-animate">
          ...
          <td>
            <button class="btn btn-sm btn-danger remove-resident-btn" data-index="${index}">
    ...
    const removeBtns = tableBody.querySelectorAll('.remove-resident-btn');
    removeBtns.forEach(btn => {
      btn.addEventListener('click', function() {
        const idx = parseInt(this.getAttribute('data-index'), 10);
        if (!isNaN(idx)) {
          // Remove from residentsData
          residentsData.splice(idx, 1);
          renderResidentsTable(residentsData);
          updateStatistics();
