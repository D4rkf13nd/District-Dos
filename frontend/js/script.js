...
fetch('get_residents.php')
  .then(res => res.json())
  .then(data => renderResidentsTable(data));
...
function addResident(resident) {
  fetch('add_resident.php', {
    ...
    if (result.success) fetchResidents();
  });
}

// Delete resident
function deleteResident(id) {
  fetch('delete_resident.php?id=' + id)
    .then(res => res.json())
    .then(result => {
      if (result.success) fetchResidents();
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // In renderResidentsTable, only animate if data.length < 100
  if (data.length < 100) {
    setTimeout(() => {
